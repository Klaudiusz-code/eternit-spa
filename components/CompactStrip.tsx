...
const features = [
  { icon: Award, text: "Certyfikowani" },
  { icon: ShieldCheck, text: "Bezpieczeństwo" },
...
export default function CompactStrip() {
  return (
    <div className="w-full border-y border-white/[0.05] bg-[#080808] py-4 md:py-8 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto px-4">
        <div className="flex flex-wrap justify-around items-center gap-3 md:gap-12">
          {features.map((item, index) => (
            <div
              key={index}
              className="group flex items-center gap-2 md:gap-3 cursor-default relative"
            >
              <div className="relative w-7 h-7 md:w-10 md:h-10 flex items-center justify-center">
                ...
                <item.icon className="w-3.5 h-3.5 md:w-5 md:h-5 text-gray-500 group-hover:text-[#C9A84C] transition-colors duration-300 relative z-10" />
              </div>

              <span className="text-[10px] md:text-sm font-light tracking-wide text-gray-400 group-hover:text-white transition-colors duration-300 whitespace-nowrap">
                {item.text}
              </span>
